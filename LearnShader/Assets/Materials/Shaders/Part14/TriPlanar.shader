Shader "Custom/TriPlanar"
{
    Properties
    {
        [NoScaleOffset]_MainTex ("Top Tex", 2D) = "white" {}
        _MainTexUV("tileU,tileV,offsetU,offsetV",vector)=(1,1,0,0)
        [NoScaleOffset]_MainTex2("Side Tex", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float2 frontUV = float2(IN.worldPos.x, IN.worldPos.y+ _Time.y);
            float2 sideUV = float2(IN.worldPos.z, IN.worldPos.y + _Time.y);
